; z80dasm 1.1.0
; command line: z80dasm -a -l -g 0 -t cpu_9733.bin
...
l000ch:
	...
	ld hl,l0398h		;003c	21 98 03 	Set up CTRC register
l003fh:
	...
	ex af,af'		;0066	08 		. 
	cp l			;0067	bd 		.
	jp nz,l0291h		;0068	c2 91 02 	Jump to Z8 - Defective CPU message routine
	ld bc,07000h		;006b	01 00 70 	Start memory test
...
l0073h:
	...
	cpl			;0077	2f 		.
	ld (hl),a		;0078	77 		.
	jp nz,l0296h		;0079	c2 96 02 	Jump to MF = Memory failure message routine
	ldi			;007c	ed a0 		Block copy
	jp pe,l0073h		;007e	ea 73 00 	loop
	ld d,0c8h		;0081	16 c8 	. . 
l0083h:
	in a,(0ffh)		;0083	db ff 		Get video status
	bit 7,a			;0085	cb 7f 		Check video RAM enabled
	jr z,l008bh		;0087	28 02 		
	in a,(0fch)		;0089	db fc 		Get keyboard data
l008bh:
	ld bc,00080h		;008b	01 80 00 	. . . 
	call sub_02d5h		;008e	cd d5 02 	Delay
	dec d			;0091	15 	. 
	jr nz,l0083h		;0092	20 ef 	 	Try again
...
l0099h:
	...
	and 00fh		;009c	e6 0f 	. . 
	or d			;009e	b2 	. 
	out (0efh),a		;009f	d3 ef 		Select drive 0, FM mode, side 0
	call sub_02dbh		;00a1	cd db 02 	. . . 
	xor a			;00a4	af 	. 
	out (0e5h),a		;00a5	d3 e5 		FDC Track Register
	ld a,005h		;00a7	3e 05 	> . 
	out (0e7h),a		;00a9	d3 e7 		FDC Data register
	ld a,013h		;00ab	3e 13 	> . 
	out (0e4h),a		;00ad	d3 e4 		FDC Command Register
	...
	out (0efh),a		;00ba	d3 ef 		? Alias of FDC Data register
	xor a			;00bc	af 	. 
	out (0c1h),a		;00bd	d3 c1 		HDA Control register
	ld a,010h		;00bf	3e 10 	> . 
	out (0c1h),a		;00c1	d3 c1 		HDA Control register
	ld a,00eh		;00c3	3e 0e 	> . 
	out (0c1h),a		;00c5	d3 c1 		HDA Control register
	ld a,0aah		;00c7	3e aa 	> . 
	out (0cah),a		;00c9	d3 ca 		WD1002 Sector Count
	in a,(0cah)		;00cb	db ca 		WD1002 Sector Count
	cp 0aah			;00cd	fe aa 	. . 
	jp nz,l01c7h		;00cf	c2 c7 01 	
	ld d,01eh		;00d2	16 1e 	. . 
	ld bc,l0000h		;00d4	01 00 00 	. . . 
l00d7h:
	in a,(0cfh)		;00d7	db cf 		WD1002 Status Register
	bit 6,a			;00d9	cb 77 		Ready bit
	jr nz,l00e3h		;00db	20 06 	  	Jump if Ready
	call sub_02d5h		;00dd	cd d5 02 	
	dec d			;00e0	15 		
	jr nz,l00d7h		;00e1	20 f4 	  	Try again
l00e3h:
	ld a,0aah		;00e3	3e aa 		
	out (0cah),a		;00e5	d3 ca 		WD1002 Sector Count
	in a,(0cah)		;00e7	db ca 		WD1002 Sector Count
	cp 0aah			;00e9	fe aa 	. . 
	jp nz,l0182h		;00eb	c2 82 01 	
	ld bc,l0438h		;00ee	01 38 04 	. 8 . 
l00f1h:
	ld a,c			;00f1	79 	y 
	out (0ceh),a		;00f2	d3 ce 		WD1002 Size/Drive/Head
	in a,(0cfh)		;00f4	db cf 		WD1002 Status Register
	bit 6,a			;00f6	cb 77 		Ready bit
	jr z,l010fh		;00f8	28 15 		Jump if not Ready
	ld a,01fh		;00fa	3e 1f 		Restore (TSR:40ms)
	out (0cfh),a		;00fc	d3 cf 		WD1002 Command Register
	call sub_0304h		;00fe	cd 04 03 	 
	xor a			;0101	af 		0
	out (0cdh),a		;0102	d3 cd 		WD1002 Cylinder High
	ld a,005h		;0104	3e 05 		5
	out (0cch),a		;0106	d3 cc 		WD1002 Cylinder Low
	ld a,073h		;0108	3e 73 		
	out (0cfh),a		;010a	d3 cf 		WD1002 Command Register
...
l010fh:
	...
	out (0cfh),a		;011d	d3 cf 		WD1002 Command Register
...
l0137h:
	xor a			;0137	af 		0
	out (0cch),a		;0138	d3 cc 		WD1002 Cylinder Low
	out (0cdh),a		;013a	d3 cd 		WD1002 Cylinder High
...
l014fh:
	in a,(0cfh)		;014f	db cf 		WD1002 Status Register
	bit 3,a		;0151	cb 5f 	. _ 
	jr z,l014fh		;0153	28 fa 		Wait until Data Request bit is set
	call sub_02f5h		;0155	cd f5 02 	Check keyboard for ESC & ETX
	jr z,l01c7h		;0158	28 6d 		Jump to floppy boot
	inc b			;015a	04 	. 
	push bc			;015b	c5 	. 
	ld a,002h		;015c	3e 02 	> . 
	ld bc,000c8h		;015e	01 c8 00 	B: 256, C: WD1002 Sector Buffer
	inir			;0161	ed b2 		Copy 1st 256 bytes
	inir			;0163	ed b2 		Copy 2nd 256 bytes
	push hl			;0165	e5 	. 
	...
	pop hl			;016e	e1 	. 
	pop bc			;016f	c1 	. 
	jr nz,l0142h		;0170	20 d0 	  	Try sector read again
	ld bc,l0000h		;0172	01 00 00 	
	call sub_02d5h		;0175	cd d5 02 	Some delay
	call sub_02f5h		;0178	cd f5 02 	Check keyboard for ESC & ETX
	jr z,l01c7h		;017b	28 4a 		Jump to floppy boot
	xor a			;017d	af 	. 
	out (0c1h),a		;017e	d3 c1 		HDA Control Register, deselect drive
	ex de,hl			;0180	eb 	. 
	jp (hl)			;0181	e9 	. 
l0182h:
...
l01ach:
	...
	ld hl,l0368h		;01b7	21 68 03 	Point to 'BOOT ERROR'
	ld de,0fb8eh		;01ba	11 8e fb 	 Point to somewhere in video memory
	ld bc,l000ch		;01bd	01 0c 00 	 message size
	ldir			;01c0	ed b0 		Do the block copy
l01c2h:
...
l01c7h:
	...
	xor a			;01ca	af 	. 
l01cbh:
	out (0c1h),a		;01cb	d3 c1 		HDA Control Register - Read/Write
	ld a,04eh		;01cd	3e 4e 		
	out (0efh),a		;01cf	d3 ef 		? alias of FDC register
	ld hl,00374h		;01d1	21 74 03 	Point to 'INSERT DISKETTE'
	ld de,0fb8eh		;01d4	11 8e fb 	 Point to somewhere in video memory
	ld bc,00011h		;01d7	01 11 00 	 message size
	ldir			;01da	ed b0 		Do the copy
l01dch:
	call sub_02dbh		;01dc	cd db 02 	Reset FDC
	bit 7,a			;01df	cb 7f 		Test NOT READY
	jr nz,l01dch		;01e1	20 f9 	  	If not ready, reset again
	ld bc,l07ffh		;01e3	01 ff 07 	Set up block
	ld de,0fffeh		;01e6	11 fe ff 	 fill action
	ld hl,0ffffh		;01e9	21 ff ff 	 of video memory
	ld (hl),020h		;01ec	36 20 		 with spaces
	lddr			;01ee	ed b8 		Clear screen
	call sub_02dbh		;01f0	cd db 02 	 Reset FDC
	ld a,00bh		;01f3	3e 0b 		Restore, Load Head, No Verify, Step Rate ?
	out (0e4h),a		;01f5	d3 e4 		FDC Command Register
	ld d,007h		;01f7	16 07 		Retry count
l01f9h:
	call sub_02d5h		;01f9	cd d5 02 	 
	in a,(0e4h)		;01fc	db e4 		FDC Status Register
	bit 0,a			;01fe	cb 47 		BUSY
	jr z,l0205h		;0200	28 03 		Jump if not
	dec d			;0202	15 		
	jr nz,l01f9h		;0203	20 f4 	  	Wait some more
l0205h:
	in a,(0e4h)		;0205	db e4 		FDC Status Register
	push af			;0207	f5 	. 
	xor 004h		;0208	ee 04 	. . 
	and 015h		;020a	e6 15 	. . 
	jr nz,l0282h		;020c	20 74 	  	Jump to DC = 'Floppy disk controller error' message
	pop af			;020e	f1 	. 
	bit 7,a		;020f	cb 7f 	.  
	jr nz,l0287h		;0211	20 74 	  	Jump to D0 = 'Drive not ready' message
	bit 3,a		;0213	cb 5f 	. _ 
	jr nz,l028ch		;0215	20 75 	  	Jump to SC = 'CRC Error' message
...
l0223h:
	...
	push de			;0224	d5 		
	push bc			;0225	c5 		
	call sub_02dbh		;0226	cd db 02 	Some delay 
	ld a,c			;0229	79 		
	out (0e6h),a		;022a	d3 e6 		FDC Sector Register
	ex af,af'		;022c	08 		
	out (0e4h),a		;022d	d3 e4 		FDC Command Register
	ex af,af'		;022f	08 		
	call sub_02d2h		;0230	cd d2 02 	Some delay
	pop bc			;0233	c1 		
	push bc			;0234	c5 		
	ld c,0e7h		;0235	0e e7 		
l0237h:
	in a,(0e4h)		;0237	db e4 		FDC Status Register
	bit 1,a			;0239	cb 4f 		DRQ?
	jr z,l0241h		;023b	28 04 		Continue if not
	ini			;023d	ed a2 		Block read sector
	jr z,l0245h		;023f	28 04 		Continue
l0241h:
	bit 0,a			;0241	cb 47 		BUSY?
	jr nz,l0237h		;0243	20 f2 	 	Retry if busy
l0245h:
	pop bc			;0245	c1 		
	pop de			;0246	d1 		
	in a,(0e4h)		;0247	db e4 		FDC Status Register
	and 01ch		;0249	e6 1c 		00011100b keep only error bits
	jr z,l025bh		;024b	28 0e 		continue if none
	pop hl			;024d	e1 		
	dec e			;024e	1d 		
	jr nz,l0223h		;024f	20 d2 	  	retry
	bit 4,a			;0251	cb 67 		SEEK ERROR
	jr nz,l029bh		;0253	20 46 	  	Jump to TK = 'Record not found bootstrap track' message routine
	bit 3,a			;0255	cb 5f 		CRC ERROR
	jr nz,l028ch		;0257	20 33 	  	Jump to SC = 'CRC Error' message routine
	jr l02a0h		;0259	18 45 		Jump to LD = 'Lost data during read' message routine
l025bh:
	pop af			;025b	f1 		
	inc c			;025c	0c 		
	...
	ld hl,l0368h+1		;0262	21 69 03 	
	ld de,01000h		;0265	11 00 10 	
	ld b,004h		;0268	06 04 		
	call sub_02edh		;026a	cd ed 02 	
	jr nz,l02a5h		;026d	20 36 	  	Jump to RS = 'Diskette not RS format' message routine
	ld hl,00385h		;026f	21 85 03 	! . . 
	ld de,01400h		;0272	11 00 14 	. . . 
	ld b,004h		;0275	06 04 	. . 
	call sub_02edh		;0277	cd ed 02 	. . . 
	jr nz,l02a5h		;027a	20 29 	  	Jump to RS = 'Diskette not RS format' message routine
	call 01404h		;027c	cd 04 14 	. . . 
	jp 01004h		;027f	c3 04 10 	. . . 
l0282h:
	ld de,04443h		;0282	11 43 44 	. C D ; DC = Floppy disk controller error.
	jr l02a8h		;0285	18 21 	. ! 
l0287h:
	ld de,04430h		;0287	11 30 44 	. 0 D ; D0 = Drive not ready
	jr l02a8h		;028a	18 1c 	. . 
l028ch:
	ld de,05343h		;028c	11 43 53 	. C S ; SC = CRC Error
	jr l02a8h		;028f	18 17 	. . 
l0291h:
	ld de,05a38h		;0291	11 38 5a 	. 8 Z ; Z8 = Defective CPU
	jr l02ach		;0294	18 16 	. . 
l0296h:
	ld de,04d46h		;0296	11 46 4d 	. F M ; MF = Memory failure
	jr l02ach		;0299	18 11 	. . 
l029bh:
	ld de,0544bh		;029b	11 4b 54 	. K T ; TK = Record not found bootstrap track
	jr l02a8h		;029e	18 08 	. . 
l02a0h:
	ld de,04c44h		;02a0	11 44 4c 	. D L ; LD = Lost data during read
	jr l02a8h		;02a3	18 03 	. . 
l02a5h:
	ld de,05253h		;02a5	11 53 52 	. S R ; RS = Diskette not RS format
l02a8h:
	ld a,04fh		;02a8	3e 4f 	> O 
	out (0efh),a		;02aa	d3 ef 		Deselect floppy drives
l02ach:
	...
	ldir		;02be	ed b0 	. . 
	in a,(0ffh)		;02c0	db ff 		Get Video state
	bit 7,a			;02c2	cb 7f 		If video RAM disabled
	jr z,l02d0h		;02c4	28 0a 		Jump to HALT
	cp 01bh			;02c6	fe 1b 		If 40 char mode & MEM bank B selected
	jp z,01004h		;02c8	ca 04 10 	Jump to boot sector?
	cp 003h			;02cb	fe 03 		If MEM bank 3 selected
	jp z,01004h		;02cd	ca 04 10 	Jump to boot sector?
l02d0h:
	halt			;02d0	76 	v 
	rst 0			;02d1	c7 	. 
sub_02d2h:
...
sub_02dbh:			;		  Reset FDC
	push bc			;02db	c5 	. 
	ld a,0d8h		;02dc	3e d8 		Force Interrupt, immediate
	out (0e4h),a		;02de	d3 e4 		FDC Command Register
	ld a,0d0h		;02e0	3e d0 		Force Interrupt, reset busy, clear Load Command and Read Status Registers
	out (0e4h),a		;02e2	d3 e4 		FDC Command Register
	call sub_0333h		;02e4	cd 33 03 	Short delay
	in a,(0e7h)		;02e7	db e7 		FDC Data Register
	in a,(0e4h)		;02e9	db e4 		FDC Status Register
	pop bc			;02eb	c1 	. 
	ret			;02ec	c9 	. 
sub_02edh:
	ld a,(de)			;02ed	1a 	. 
	...
	ret			;02f4	c9 	. 
sub_02f5h:
	in a,(0ffh)		;02f5	db ff 		Get Video status
	xor 080h		;02f7	ee 80 	. . 
	bit 7,a			;02f9	cb 7f 		Is video RAM enabled?
	ret nz			;02fb	c0 		Return if true
	in a,(0fch)		;02fc	db fc 		Read keyboard
	cp 01bh			;02fe	fe 1b 		Is it ESC?
	ret z			;0300	c8 		Return if so
	cp 003h			;0301	fe 03 		Is it ETX?
	ret			;0303	c9 	. 
sub_0304h:
	push bc			;0304	c5 	. 
	ld bc,07fffh		;0305	01 ff 7f 	. .  
l0308h:
	in a,(0cfh)		;0308	db cf 		WD1002 Status Register
	bit 7,a			;030a	cb 7f 		Busy bit
	jr nz,l0312h		;030c	20 04 	  	
	bit 4,a			;030e	cb 67 		Seek Complete bit
	jr nz,l031fh		;0310	20 0d 	  . 
l0312h:
	ex (sp),ix		;0312	dd e3 		Very short delay?
	ex (sp),ix		;0314	dd e3 	. . 
	dec bc			;0316	0b 	. 
	ld a,b			;0317	78 	x 
	or c			;0318	b1 	. 
	jr nz,l0308h		;0319	20 ed 	  	Try Again
	or 008h		;031b	f6 08 	. . 
	pop bc			;031d	c1 	. 
	ret			;031e	c9 	. 
l031fh:
...
l0368h:
	...
	ld b,c			;0387	41 	A 
	ld b,a			;0388	47 	G 
	ld h,e			;0389	63 	0  Horizontal Total
	ld d,b			;038a	50 	1  Horizontal Displayed (80 char.)
	ld d,l			;038b	55 	2  H. Sync Position
	ex af,af'		;038c	08 	3  H. Sync Width
	add hl,de		;038d	19 	4  Vertical Total (25 lines)
	nop			;038e	00 	5  V. Total Adjust
	jr l03a9h		;038f	18 18 	7  V.Sync Position, 6  Vertical Displacement
l0391h:
	nop			;0391	00 	8  Interlace Mode
	add hl,bc		;0392	09 	9  Max Scan Line Address
	ld h,l			;0393	65 	a  Cursor Start
	add hl,bc		;0394	09 	b  Cursor End
l0395h:
	nop			;0395	00 	c  Start Address (H)
	nop			;0396	00 	d  Start Address (L)
	inc bc			;0397	03 	e  Cursor (H)
l0398h:
	jp (hl)			;0398	e9 	f  Cursor (L)

	dec b			;0399	05 	Version
	add a,e			;039a	83 	YY
	rlca			;039b	07 	MM
	add hl,hl		;039c	29 	DD
	dec d			;039d	15 	. 
	nop			;039e	00 	. 
	nop			;039f	00 	. 
	nop			;03a0	00 	. 
